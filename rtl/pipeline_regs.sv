...
module id_ex_reg (
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst || flush) begin
            pc_ex <= 0; rs1_data_ex <= 0; rs2_data_ex <= 0; imm_ex <= 0;
            rs1_addr_ex <= 0; rs2_addr_ex <= 0; rd_addr_ex <= 0;
            alu_ctrl_ex <= 0; reg_write_ex <= 0; mem_read_ex <= 0;
            mem_write_ex <= 0; mem_to_reg_ex <= 0; alu_src_ex <= 0; branch_ex <= 0;
        end else begin
            pc_ex <= pc_id; rs1_data_ex <= rs1_data_id; rs2_data_ex <= rs2_data_id; imm_ex <= imm_id;
            rs1_addr_ex <= rs1_addr_id; rs2_addr_ex <= rs2_addr_id; rd_addr_ex <= rd_addr_id;
            alu_ctrl_ex <= alu_ctrl_id; reg_write_ex <= reg_write_id; mem_read_ex <= mem_read_id;
            mem_write_ex <= mem_write_id; mem_to_reg_ex <= mem_to_reg_id; alu_src_ex <= alu_src_id; branch_ex <= branch_id;
        end
    end
endmodule
...
module ex_mem_reg (
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            alu_result_mem <= 0; rs2_data_mem <= 0; rd_addr_mem <= 0;
            reg_write_mem <= 0; mem_read_mem <= 0; mem_write_mem <= 0; mem_to_reg_mem <= 0;
        end else begin
            alu_result_mem <= alu_result_ex; rs2_data_mem <= rs2_data_ex; rd_addr_mem <= rd_addr_ex;
            reg_write_mem <= reg_write_ex; mem_read_mem <= mem_read_ex; mem_write_mem <= mem_write_ex; mem_to_reg_mem <= mem_to_reg_ex;
        end
    end
endmodule
...
module mem_wb_reg (
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            alu_result_wb <= 0; mem_read_data_wb <= 0; rd_addr_wb <= 0;
            reg_write_wb <= 0; mem_to_reg_wb <= 0;
        end else begin
            alu_result_wb <= alu_result_mem; mem_read_data_wb <= mem_read_data_mem; rd_addr_wb <= rd_addr_mem;
